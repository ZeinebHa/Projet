...
int main(void) 
{
	...
	SDL_WM_SetCaption("Corona Dodge",NULL);
	...
	while (continuer) 
   	{
		get_input_menu(&inp,m,&continuer,sound,&barrougeX);
		update_menu(&m,&inp,&continuer);
		
		if (m.intro==1)//logo nightwalkers
		{
		...
		m.choix_options=-1;//si on la met dans update la gestion du son ne fonctionne plus
		SDL_Flip(ecran);	
	}
