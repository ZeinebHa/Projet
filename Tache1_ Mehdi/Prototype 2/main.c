...
SDL_Rect positionPerso,positionmasque,positionback,saut;
SDL_Event event;
personnage p;
int continuer = 1,r,z;
int quit =1;
...
j++;
x=p.position.y+73;
SDL_GetRGB(GetPixel(ecran,p.position.x,x),backmask->format,&r1,&g,&bl);
			
break;
case SDLK_d: // Flèche gauche

 jump(&p,back,ecran,backmask,positionback);

			printf("yh :%d\n",p.position.y);
			
/*p.position.x = jumpright (ecran,p.position,positionback);
p.position.y = jumpright (ecran,p.position,positionback);*/
...
p.position.x += vecteur.x;
p.position.y += vecteur.y;
break;
}
break;
}
